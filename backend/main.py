...
from config import BATCH_OPTIONS
...
# Wire detector into session_manager so it can access the event loop
_session_manager.detector = detector


@app.on_event("startup")
async def _startup():
    """
    Capture the running FastAPI event loop and pass it to AsyncDetection.
    This enables run_coroutine_threadsafe() calls from background threads.
    """
...
async def register_student(
    ...
    try:
        ...
        if success:
            # Persist to MongoDB (no raw embedding bytes — pkl is the embedding store)
...
# ============================================================
# MULTI-POSE STUDENT REGISTRATION (NEW — 5 poses via browser webcam)
# ============================================================

@app.post("/register_student_multi")
async def register_student_multi(
    ...
    """
    Multi-pose student registration for improved face recognition accuracy.
    Accepts up to 5 pose images (front, left, right, up, down).
    At least 1 image is required. More poses = higher accuracy.
    """
    async def _read_frame(upload: UploadFile):
        if upload is None:
            return None
        try:
    ...
    pose_frames = {}
    for pose_name, upload in [
        ("front", front), ("left", left), ("right", right),
        ("up", up), ("down", down)
    ]:
        frame = await _read_frame(upload)
        if frame is not None:
            pose_frames[pose_name] = frame

    if not pose_frames:
        return {"success": False, "message": "No valid images provided"}

    try:
        success, message = detector.face_manager.register_face_multi(
            name, batch, roll_no, prn, pose_frames
        )
        if success:
            shared_state.activity_log.insert(0, {
                "timestamp": datetime.now().strftime("%H:%M:%S"),
                "message": f"Student {name} registered with {len(pose_frames)} pose(s)",
                "type": "registration"
            })
        return {"success": success, "message": message, "poses_used": list(pose_frames.keys())}
    except Exception as e:
        return {"success": False, "message": str(e)}


# ============================================================
# STUDENT MANAGEMENT (PRESERVED)
# ============================================================
...
async def get_attendance():
    """!! PRESERVED: Get all attendance records from MongoDB !!"""
...
def get_attention():
    """Get live attention data for all tracked students (enhanced with head pose + visibility)."""
    result = []
...
def dashboard_stats():
    ...
    return {
        ...
        # Compute unknown_faces from actual latest detection results
        "unknown_faces": shared_state.unknown_faces_count,
...
# ============================================================
# CONFIG ENDPOINTS (dynamic settings from .env)
# ============================================================

@app.get("/config/batches")
def get_batch_options():
    """
    Return configured batch/division options.
    Set BATCH_OPTIONS env var to customize: e.g. "SE-A,SE-B,TE-A,TE-B"
    """
    return {"batches": BATCH_OPTIONS}
