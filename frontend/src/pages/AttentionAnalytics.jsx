...
export default function AttentionAnalytics() {
  ...
  useEffect(() => {
    fetchSessions();
    fetchLive(); // Initial live data fetch
    // Subsequent live updates come via WS attention_update event below
  ...
  // WebSocket: real-time attention score updates (replaces 4s poll)
  useWebSocket("attention_update", (data) => {
    if (selectedSession === "live") {
      setStudents(data.students || []);
      setAvgAttention(data.avg_score || 0);
      setTotalTracked((data.students || []).length);
    }
  });
                  ...
                  <tbody>
                    {students.map((s, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          {s.head_pose_score != null
                            ? <span style={{ fontSize: "0.75rem", color: s.head_pose_score >= 0.6 ? "var(--color-green)" : "var(--color-orange)" }}>
                                {Math.round(s.head_pose_score * 100)}%
                              </span>
