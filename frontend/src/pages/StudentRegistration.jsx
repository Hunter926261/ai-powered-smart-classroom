...
import { getStudents, registerStudent, deleteStudent, getBatchOptions, VIDEO_FEED_URL } from "../services/api";
...
export default function StudentRegistration() {
  ...
  // Batch options loaded from backend /config/batches (dynamic, not hardcoded)
  const [batchOptions, setBatchOptions] = useState([]);
  const fileRef = useRef();

  // Fetch batch options from backend config
  const fetchBatches = async () => {
    try {
      const { data } = await getBatchOptions();
      setBatchOptions(data.batches || []);
    } catch {
      setBatchOptions(["1", "2", "3", "A", "B"]);
    }
  };
                  ...
                  <select className="form-select" value={batch} onChange={e => setBatch(e.target.value)}>
                    ...
                    {batchOptions.map(b => (
                      <option key={b} value={b}>
                        {b.length === 1 && !isNaN(b) ? `Batch ${b}` : `Division ${b}`}
                      </option>
                    ))}
                  </select>
                </div>
